...
import com.cosmic.launcher.asm.AnnotationVisitor;
import com.cosmic.launcher.asm.Attribute;
import com.cosmic.launcher.asm.ClassVisitor;
import com.cosmic.launcher.asm.ConstantDynamic;
import com.cosmic.launcher.asm.Context;
import com.cosmic.launcher.asm.FieldVisitor;
import com.cosmic.launcher.asm.Handle;
import com.cosmic.launcher.asm.Label;
import com.cosmic.launcher.asm.MethodVisitor;
import com.cosmic.launcher.asm.MethodWriter;
import com.cosmic.launcher.asm.ModuleVisitor;
import com.cosmic.launcher.asm.Opcodes;
import com.cosmic.launcher.asm.RecordComponentVisitor;
import com.cosmic.launcher.asm.Type;
...
public class ClassReader {
    ...
    ClassReader(byte[] classFileBuffer, int classFileOffset, boolean checkClassVersion) {
        ...
        if (checkClassVersion && this.readShort(classFileOffset + 6) > 67) {
            throw new IllegalArgumentException(ClassReader.stringConcat$0(this.readShort(classFileOffset + 6)));
        }
    ...
    private static /* synthetic */ String stringConcat$0(short s) {
        return "Unsupported class file major version " + s;
    }
    ...
    public ClassReader(String className) throws IOException {
        this(ClassReader.readStream(ClassLoader.getSystemResourceAsStream(ClassReader.stringConcat$1(className.replace('.', '/'))), true));
    }
