...
async function main() {
    ...
    for (let i = 0; i < manifest.files.length; i++) {
        ...
        if (file.id === 'natives-linux-x64') {
            const linuxNativesDir = path.join(COSMIC_CLIENT_DIR, '1.8', 'bin-linux');
            console.log(` -> Extracting Linux natives to ${linuxNativesDir}`);
            await extractTarGz(archiveFile, linuxNativesDir);
        }

        if (file.id === 'libraries') {
            const libsDir = path.join(COSMIC_CLIENT_DIR, '1.8', 'libraries');
            console.log(` -> Extracting Libraries to ${libsDir}`);
            await extractTarGz(archiveFile, libsDir);
        }
    ...
    console.log('[4/4] Cross-platform offline archive complete!');
    console.log('Archive stored at:', TARGET_ARCHIVE_DIR);
}
