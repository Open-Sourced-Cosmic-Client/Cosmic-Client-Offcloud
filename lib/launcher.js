...
async function launch(installDir, opts = {}, onLog = () => {}, onExit = () => {}) {
    ...
    try {
        ...
        if (agentPath && fs.existsSync(agentPath) && path.resolve(agentPath) !== path.resolve(destAgent)) {
            if (!fs.existsSync(destAgent) || fs.statSync(destAgent).size !== fs.statSync(agentPath).size || fs.statSync(destAgent).mtimeMs < fs.statSync(agentPath).mtimeMs) {
